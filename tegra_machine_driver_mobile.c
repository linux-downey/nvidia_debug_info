					...
					bool is_playback)
{
	...
	printk(KERN_INFO "enter hw clk param setup!\n");
	printk(KERN_INFO "========================================!\n");
	...
	if (rtd) {
		...
		printk(KERN_INFO  "1111111111111111111\n");
		dai_params =
	...
	if (rtd) {
		printk(KERN_INFO  "222222222222222222222\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	if (rtd) {
		printk(KERN_INFO  "3333333333333333333333333\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	if (rtd) {
		printk(KERN_INFO  "4444444444444444444444\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	if (rtd) {
		printk(KERN_INFO  "55555555555555555555555\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	if (rtd) {
		printk(KERN_INFO  "6666666666666666666666666\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	if (rtd) {
		printk(KERN_INFO  "777777777777777777777\n");
		dai_params =
		(struct snd_soc_pcm_stream *)rtd->dai_link->params;
	...
	if(rtd){
		printk(KERN_INFO "enter wm8960xxx!\n");
		 dai_params =
			(struct snd_soc_pcm_stream *)rtd->dai_link->params;
		  dai_params->rate_min = clk_rate;
		  dai_params->channels_min = channels;
	...
	printk(KERN_INFO "clk_out_rate = %d\n",(unsigned int)clk_out_rate);
	return 0;
}
...
static int tegra_machine_wm8960_init(struct snd_soc_pcm_runtime *rtd)
{
	 ...
	 int err;
	  err = snd_soc_dai_set_sysclk(rtd->codec_dai, WM8960_SYSCLK_MCLK, 12288000, SND_SOC_CLOCK_IN);
	  if (err) {
	...
	for (i = 0; i < machine->num_codec_links; i++) {
		if (tegra_machine_codec_links[i].name) {
			if (strstr(tegra_machine_codec_links[i].name,
				...
				tegra_machine_codec_links[i].init =
					tegra_machine_dspk_init;
			else if (strstr(tegra_machine_codec_links[i].name,"wm8960")){
				 printk(KERN_INFO "enter wm8960 init set!\n");
				 tegra_machine_codec_links[i].init =
